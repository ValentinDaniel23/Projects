...
public class MovieWindow extends JFrame {
    ...
    public MovieWindow(String name) {
        setTitle("IMDB Movie");
        setSize(700, 500);
        ...
        Movie movie = (Movie) IMDB.findProduction(name);
        ...
        stringBuilder.append("Plot: " + movie.getDescription() + "\n\n");
        stringBuilder.append(movie.getReleaseYear() + " : " + movie.getDuration());
        List<String> directors = movie.getDirectorsName();
        ...
        plusButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                CommentWindow commentWindow = new CommentWindow(movie);
            }
        });
        JLabel noteLabel = new JLabel("Avg rating: " + movie.getAverageRating().toString() + "/10");
        noteLabel.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                Collections.sort(movie.getRatings());
                NotesWindow notesWindow = new NotesWindow(movie.getRatings());
            }
        });
        ...
        addToFavoritesButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                boolean found = false;
                if (IMDB.user.getFavorites().contains(movie)) found = true;

                if (found == true) {
                    JOptionPane.showMessageDialog(MovieWindow.this,
                            "Already in your favorite list",
                            "Add to favorite list",
                            JOptionPane.INFORMATION_MESSAGE);
                }
                else {
                    IMDB.user.addToFavorites(movie);
                    JOptionPane.showMessageDialog(MovieWindow.this,
        ...
        for (String actor : movie.getActorsName()) {
        ...
        if (IMDB.user.getAccountType() != AccountType.ADMIN) {
            ...
            reportButton.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    String to = IMDB.getProductionUser(movie).getUsername();
